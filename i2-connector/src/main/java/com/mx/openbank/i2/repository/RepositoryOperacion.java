...
public interface RepositoryOperacion extends JpaRepository<Operacion, String> {
    ...
    @Query(
        ...
        value =
        "SELECT " +
        "   op.transaction_ref as id, " +
        "   op.transaction_ref AS transactionRef, " +
        "   op.beneficiary_customer_id AS beneficiaryCustomerId, " +
        "   op.cust_id AS custId, " +
        "   op.operation_code AS operationCode, " +
        "   CAST(replace(op.external_address2, ',', ' ') AS TEXT) AS externalAddress2, " +
        ...
        "  AND (:Codigosoperacionentrada IS NULL OR CAST(op.operation_code AS TEXT) = :Codigosoperacionentrada) " +
        "  AND (:Codigosoperacionsalida IS NULL OR CAST(op.operation_code AS TEXT) = :Codigosoperacionsalida) " +
        "  AND (:LatitudLongitud IS NULL OR op.external_address2 = :LatitudLongitud)"
    )
    List<Operacion> buscarOperaciones(
        @Param("IDtransaccion") String transactionId,
        @Param("Codigosoperacionentrada") String codigoEntrada,
        @Param("Codigosoperacionsalida") String codigoSalida,
        @Param("LatitudLongitud") String latLon
    );
}
